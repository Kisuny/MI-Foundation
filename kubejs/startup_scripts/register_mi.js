...
function cool(part) {
  if (part === 'hot_ingot') { return 'ingot' } else { return part }
}

function hardnessCoeff(string) {
  let lookup = {
    "soft": 0.5,
    "average": 1,
    "hard": 2,
...
const liquid_metals = [
  {
    ...
    materialset: 'shiny',
    hardness: 'soft', 
    parts: ['hot_ingot' ,'dust', 'rod', 'plate', 'wire', 'fine_wire'],
    external: [ 
      {
	type: 'raw_metal',
...
liquid_metals.forEach(metal => {
  ...
  all_parts.forEach(part => {
    JsonIO.write(`kubejs/data/modern_industrialization/recipes/smeltery/${metal.name}_${part}.json`,
      {
	'type': 'modern_industrialization:smeltery',
	'eu': 32 * hardnessCoeff(metal.hardness),
	'duration': 200,
	'item_inputs': [
	  {
	...
	'fluid_outputs': [
	  {
	    ...
	    'amount': molten_lookup_table[part]
	  }
	]
      }
    )
  })
  let with_molds = all_parts.filter(p => (metal_molds.includes(p)));
  if (with_molds?.length) {
    // ('hot_ingot' in all_parts) ? with_molds.filter(p=> p !== 'ingot').push('hot_ingot') : {};
    with_molds.forEach(part => {
      console.log(part)
      // Still can't figure out how to reconsile the hot ingots
      JsonIO.write(`kubejs/data/modern_industrialization/recipes/fluid_solidifier/${metal.name}_${part}.json`,
	{
	  ...
	  'eu': 16 * hardnessCoeff(metal.hardness),
	  'duration': 200,
	  'fluid_inputs': [
	    {
	      'fluid': `modern_industrialization:molten_${metal.name}`,
...
MIMaterialEvents.addMaterials(event => {
  ...
  event.createMaterial('Tungsten Hexachloride', 'tungsten_hexachloride', 0x05122D, builder => {
    builder
      .addParts('dust','tiny_dust')
      .defaultRecipes();
  });

  event.createMaterial('Tungsten Carbide', 'tungsten_carbide', 0xC0C0C0, builder => {
    builder
